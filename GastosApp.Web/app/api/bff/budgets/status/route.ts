...
import { normalizeBudgetStatuses, resolvePeriodParam } from "@/lib/contracts/budgets";
...
export async function GET(request: Request) {
  ...
  const { response, session: updatedSession } = await fetchApiWithAutoRefresh(
    session,
    `${getApiBaseUrl()}/api/budgets/status?period=${encodeURIComponent(period.data)}`,
    {
      method: "GET",
      cache: "no-store"
    }
  ...
  if (!response.ok) {
    const message = response.status === 401 ? "Session expired" : "Failed to fetch budget statuses";
  ...
  const out = NextResponse.json(normalizeBudgetStatuses(raw));
  await attachSessionCookie(out, updatedSession, session);
  return out;
}
