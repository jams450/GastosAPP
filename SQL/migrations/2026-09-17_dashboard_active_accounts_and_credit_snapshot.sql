...
-- Fase 1 — Dashboard financiero: snapshot de crédito y exclusión de cuentas inactivas.
--
-- Contexto de reproducibilidad:
--   * accounts.credit_limit lo mapea GastosApp.Models.Entities.Account y lo usa
--     fn_dashboard_credit_overview, pero no lo creaba ni el bootstrap ni ninguna migración.
--   * fn_dashboard_credit_overview sólo existía en migraciones, no en SQL/schema.sql.

-- 1) Columna de límite de crédito (idempotente).
ALTER TABLE accounts
    ADD COLUMN IF NOT EXISTS credit_limit DECIMAL(10, 2);

-- 2) Una cuenta de crédito activa sin límite (> 0) no debe persistir: sin límite no hay
--    disponible calculable. Las filas históricas inválidas se archivan (active = FALSE) en
--    lugar de borrarse, para no perder movimientos; quedan excluidas del dashboard por activo.
UPDATE accounts
SET active = FALSE,
...
-- El CHECK sólo exige límite a las cuentas de crédito activas: una fila archivada puede
-- conservarse como estaba y no puede reactivarse sin límite válido.
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'chk_accounts_credit_limit'
    ) THEN
        ALTER TABLE accounts
...
-- 3) Índices del dashboard (idempotentes).
...
-- 4) fn_dashboard_credit_overview vigente (2026-08-21_credit_cycles_due_alignment)
--    + filtro de cuentas activas. Los CTE derivados heredan el filtro vía accounts_scope.
